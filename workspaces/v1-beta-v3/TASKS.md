...
- [x] [Blueprint] Fix baton checkpoint sync for EscalationResolved/EscalationTimeout/CancelJob/ShutdownRequested (priority: P2) [source: F-211, Weaver M3] — Extended _sync_sheet_status to handle ALL single-sheet events via duck typing (hasattr job_id+sheet_num), CancelJob via pre-event capture (job deregistered before sync), ShutdownRequested (non-graceful) via direct state read. 18 TDD tests in test_f211_checkpoint_sync_gaps.py.
