...
1. **F-271 — MCP Process Explosion** (`src/marianne/execution/instruments/cli_backend.py:229-233`)
   ...
   - I replaced this with a profile-driven approach: new `CliCommand.mcp_disable_args` field (`src/marianne/core/config/instruments.py:172-177`) that each instrument defines in its YAML profile.
...
2. **F-255.2 — Baton _live_states Not Populated** (`src/marianne/daemon/manager.py:2017-2038`)
...
**The flip itself:** (`src/marianne/daemon/config.py:333-338`)
...
Checked all 9 daemon/profiler config models at `src/marianne/daemon/config.py` and `src/marianne/daemon/profiler/models.py`. Every BaseModel subclass already has `model_config = ConfigDict(extra="forbid")`. No work needed.
...
| File | Change |
|------|--------|
| `src/marianne/daemon/config.py:333-338` | `use_baton` default False→True, updated description |
| `src/marianne/core/config/instruments.py:172-177` | Added `mcp_disable_args` field to CliCommand |
| `src/marianne/execution/instruments/cli_backend.py:229-233` | Profile-driven MCP disable injection |
| `src/marianne/daemon/manager.py:2031-2032` | Added instruments_used/total_movements to initial state |
| `src/marianne/instruments/builtins/claude-code.yaml:79-85` | Added mcp_disable_args to profile |
