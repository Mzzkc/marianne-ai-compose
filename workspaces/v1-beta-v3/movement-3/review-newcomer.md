...
I approached Mozart as if I discovered it on GitHub thirty seconds ago. `mozart doctor` is one of the best health-check commands I've seen in any CLI tool — it tells you what's ready, what's missing, and what you should configure, all in one screen. `mozart validate examples/hello.yaml` produces the best dry-run visualization I've encountered: a DAG showing parallel execution, a prompt preview, and validation expansion per sheet. Error messages for garbage input are genuinely helpful — they tell you what went wrong, what you should do instead, and where to read more. The no-args `mozart status` overview is exactly `git status` for orchestration work. This product surface is better than 90% of the CLI tools I encounter.
...
The hint saves you. But the hint shouldn't have been necessary. The score's `name:` field is `hello-mozart`. The conductor registered it under the ID `hello` (derived from the filename `hello.yaml`). The README teaches you the name. The conductor expects the ID. The flagship example is the one that exposes the design flaw.
...
$ mozart doctor
  Py 3.12.3, Mozart v0.1.0, Conductor running (pid 1277279)
...
**Grade: A-.** The grouping (Getting Started, Jobs, Monitoring, Diagnostics, Conductor, Services, Configuration & Learning) is intuitive. One discoverability gap: `--conductor-clone` is a global option visible on `mozart --help` but does NOT appear in `mozart start --help`. A newcomer looking for "how to start a clone conductor" would naturally look at `start --help` first, find nothing, and conclude the feature doesn't exist. This is a discoverability gap, not a bug — but it's the kind of friction that prevents adoption of safety features.
...
- `examples/hello.yaml:16` — `#   mozart status hello-mozart`

Four locations. All wrong. Root cause: issue #124 (score name vs filename-derived ID).

**Key observation:** No partial or fuzzy matching exists. `mozart status hello-m` also fails with the same error. The conductor does exact ID matching only. Adding fuzzy matching or accepting the `name:` field would fix the root cause.

### 6. F-450 — Independent Reproduction

```
$ mozart conductor-status   -> implied running (via doctor)
$ mozart doctor             -> Conductor running (pid 1277279)
$ mozart status             -> RUNNING (4 active)
$ mozart clear-rate-limits  -> Error: Mozart conductor is not running
```

Three commands confirm the conductor is running. One says it isn't. The error tells you to `mozart start` — but the conductor is already started. This is the worst error message in the product. It doesn't just fail to help — it actively misleads. The user would rationally conclude they misunderstand the system. Root cause: the `clear_rate_limits` IPC method was added in M3 code but the running conductor was started from pre-M3 code. "Method not found" and "conductor not running" are indistinguishable in the IPC error path.
...
| Partial score name | "Score not found" + "Run 'mozart list'" | **B** (no fuzzy suggestion) |
...
$ mozart init --path /tmp/newcomer-test --name fresh-test
  ...
  Next steps:
    ...
    3. mozart status fresh-test
```

**Grade: A.** The generated score validates clean. Step 3 says `mozart status fresh-test` which would WORK because `init` sets `name: fresh-test` to match the filename `fresh-test.yaml`. The init path is designed to avoid the identity bug (F-465). Users who start with `init` will have a clean experience. Users who start with the README will not. The onboarding path that showcases the product is the one that breaks.
...
1. **No fuzzy/partial score ID matching.** `mozart status hello-m` fails just like `hello-mozart`. There's no "did you mean...?" suggestion. The hint says "Run 'mozart list'" but doesn't suggest the closest match. This is a missed UX opportunity that would turn F-465 from a hard error into a gentle redirect.

2. **The `docs/getting-started.md` has a split personality.** Line 60 uses `hello-mozart` (wrong, F-465). Lines 198+ use `my-first-score` (correct by design). The guide teaches you the wrong pattern first, then switches to the right pattern. A newcomer who followed the hello section and got confused would then encounter the my-first-score section using the correct pattern and wonder why the first one failed.

3. **`--conductor-clone` is invisible where it matters most.** The flag appears in `mozart --help` (global options) but NOT in `mozart start --help`. A newcomer who wants to start a clone conductor would check `start --help`, find no mention, and assume the feature doesn't exist. The composer's P0 directive requires `--conductor-clone` for all testing, but the most natural discovery path doesn't reveal it.
...
1. **`mozart doctor`** — Best health-check experience I've seen. Immediate understanding of what's ready.
2. **`mozart validate`** — Best dry-run visualization in any orchestration tool. DAG + prompt preview + per-sheet validation expansion.
3. **`mozart status` (no args)** — Perfect overview. `git status` for orchestration.
...
1. **Fix the quick start (F-465).** Change ALL FOUR locations to use `hello` instead of `hello-mozart`. This is a 4-line fix that eliminates the #1 newcomer friction. Separately, fix #124 so the conductor accepts score names — that's the real fix.
