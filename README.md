...
Mozart ships with `examples/hello-mozart.yaml` — a score that creates an interconnected fiction collection in three movements: a world setting, three parallel character vignettes, and a finale.

```bash
mozart validate examples/hello-mozart.yaml
```
...
| Example | Description |
|---------|-------------|
| [hello-mozart.yaml](examples/hello-mozart.yaml) | Your first score — interconnected fiction in three movements with parallel voices |
...
Side-by-side comparisons with [Wordware's](https://wordware.ai) published use cases — declarative YAML vs IDE-based prompt chaining.

| Example | Domain | Description |
|---------|--------|-------------|
| [contract-generator.yaml](examples/contract-generator.yaml) | Legal | Parallel contract section generation with cross-reference validation |
| [candidate-screening.yaml](examples/candidate-screening.yaml) | HR | Multi-candidate parallel evaluation against weighted criteria |
| [marketing-content.yaml](examples/marketing-content.yaml) | Marketing | 4-voice parallel channel content with brand consistency audit |
| [invoice-analysis.yaml](examples/invoice-analysis.yaml) | Finance | 3-voice parallel expert analysis (financial, compliance, anomaly) |
